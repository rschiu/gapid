 ...
 */
public class SfEventsPanel extends TrackPanel implements Selectable {
  private static final double SLICE_HEIGHT = 30;// - 2 * TRACK_MARGIN;
  private static final double HOVER_MARGIN = 10;
  private static final double HOVER_PADDING = 4;

  private final SfEvents.Queue queue;
  ...
  public SfEventsPanel(State state, SfEvents.Queue queue, SliceTrack track) {
    super(state);
    this.queue = queue;
    this.track = track;
  }
  ...
  protected Hover onTrackMouseMove(Fonts.TextMeasurer m, double x, double y) {
    ...
    for (int i = 0; i < data.starts.length; i++) {
      ...
      if (data.depths[i] == depth && x >= ts && x<= ts + 16 ) {
